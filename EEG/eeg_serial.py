...
RAW_FEAT_NAMES = [
    ...
    'beta_ch0', 'beta_ch1', 'beta_ch2',
    'HFD_ch0', 'HFD_ch1', 'HFD_ch2',
    'alpha_ch0', 'alpha_ch1', 'alpha_ch2',
    'theta_ch0', 'theta_ch1', 'theta_ch2',
]

# Python 端从上面 16 维计算出的衍生特征 (8 维)
DERIVED_FEAT_NAMES = [
    'theta_alpha_r0', 'theta_alpha_r1', 'theta_alpha_r2',
    'FBA',
    'rel_alpha_ch0', 'rel_alpha_ch1', 'rel_alpha_ch2',
    'theta_beta_mid',
]
...
def compute_derived_features(alpha: List[float],
                              beta: List[float],
                              theta: List[float]) -> List[float]:
    """
    ...
    # θ/α 比 (3 个通道)
    theta_alpha_r = [
        theta[c] / max(alpha[c], EPS) for c in range(3)
    ]

    # β 不对称 (FBA = ln(β_right) - ln(β_left))
    bL = max(beta[0], EPS)  # ch0 = left
    bR = max(beta[1], EPS)  # ch1 = right
    fba = math.log(bR) - math.log(bL)
...
if __name__ == '__main__':
    ...
    def _on_feat(f: FeaturePacket):
        v = f.to_vector()
        print(f"[FEAT] {len(v)}维  FAA={f.FAA:+.4f}  "
              f"HFD=[{f.HFD[0]:.3f},{f.HFD[1]:.3f},{f.HFD[2]:.3f}]")

    rx = EEGSerialReceiver(port, on_feat=_on_feat)
    rx.start()
    try:
        while True:
            time.sleep(5)
