...
# 为了避免触发接口限流，在每次调用 LLM 之间增加等待时间。
# 可通过环境变量 QA_REQUEST_INTERVAL 动态调整，单位秒。
REQUEST_INTERVAL = float(os.getenv('QA_REQUEST_INTERVAL', '1.0'))
...
def single_thread_generate(thread_num, interval, model_caller, storage_jsonl_path, contents):
    ...
    for content in tqdm(contents):
        try:
            ...
            # 无论调用是否成功，都等待一段时间，防止短时间内连续请求导致 429 限流
