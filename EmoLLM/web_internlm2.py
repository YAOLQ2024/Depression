...
from rag.src.config.config import default_vector_db_key, vector_db_presets, rerank_flag as default_rerank_flag
...
def get_rag_instance(model, selected_kb: str):
    ...
    if selected_kb not in rag_cache:
        ...
        with st.spinner(f"正在加载知识库：{selected_kb}"):
            try:
                # 使用 config.py 中的配置：retrieval_num=10, select_num=5, rerank_flag=True
                rag_cache[selected_kb] = EmoLLMRAG(
                    model, 
                    vector_db_path=target_path,
                    rerank_flag=default_rerank_flag
                )
